...
class PurchaseOrder(models.Model):
    ...
    lines_discount_amount = fields.Float(string="Lines Discount", digits="Discount")
    discount_amount = fields.Float(string="Total Discount", digits="Discount")

    @api.depends('order_line.price_total', 'order_line.discount', 'discount_amount')
    def _amount_all(self):
        super(PurchaseOrder, self)._amount_all()
        for order in self:
            lines_discount_amount = sum(line.price_unit * (line.discount / 100) for line in order.order_line)
            modifications = {'lines_discount_amount': lines_discount_amount}
            if order.discount_amount:
                modifications['amount_total'] = order.amount_total - order.discount_amount
